...
export default function Olimpiada() {
  ...
  const handleChangeCity = (event) => {
    ...
    if (event.target.value === "Todos") setResultado2(resultado);
      ...
      var city = event.target.value;
      var val = resultado.filter((r) => r.distrito.departamento.includes(city));
      setBuscar("");
      setResultado2(val);
    }
  };
  ...
  const Update = async (event) => {
    ...
    await axios
      .patch(
        baseUrl + "/" + consoleSeleccionada.sie,
        JSON.stringify({
          nombre: consoleSeleccionada.nombre,
          zona: consoleSeleccionada.zona,
          direccion: consoleSeleccionada.direccion,
  ...
  const Delete = async (event) => {
    ...
    await axios
      .delete(baseUrl + "/" + consoleSeleccionada.sie, header)
      .then((response) => {
        //showNotificationSuccess('success','Grupo guardado con exito');
  ...
  const getAllDistritos = useCallback(async () => {
    await axios
      .get(baseUrlDistrito, JSON.stringify({}), header)
      .then((response) => {
    ...
    <div>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
          <TableHead>
            <TableRow>
              <StyledTableCell align="left">Sie</StyledTableCell>
              ...
              <StyledTableCell align="left">Dependencia</StyledTableCell>
