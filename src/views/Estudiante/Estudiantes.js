...
export default function Olimpiada() {
  ...
  const handleChangeSearch = (event) => {
    ...
    if(1>event.target.value.length)
      ...
      var val = resultado.filter( 
        r => (r.rude+r.nombre+r.apPaterno+r.apMaterno+r.celular+r.fechaNac+r.genero+r.ci+r.correo).toLowerCase().includes(city.toLowerCase())
      );
      setResultado2(val);
    }
  };
  ...
  const Insert=async(event)=>{
    ...
    const Update=async(event)=>{
      ...
      await axios.patch(baseUrl+"/"+consoleSeleccionada.rude,
        JSON.stringify({
          "nombre":consoleSeleccionada.nombre,
          "apPaterno": consoleSeleccionada.apPaterno,
          "apMaterno": consoleSeleccionada.apMaterno,
  ...
  const Delete=async(event)=>{
    ...
    await axios.delete(baseUrl+"/"+consoleSeleccionada.rude
      ,header
    ).then(
      response => {
        console.log(response);
  ...
  return (
    <div >
      ...
      <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
        <TableHead>
          <TableRow>
            <TableCell style={{ width: 10 }}>RUDE</TableCell>
            <TableCell align="left">nombre</TableCell>
            <TableCell align="left">apPaterno</TableCell>
            <TableCell align="left">apMaterno</TableCell>
            <TableCell align="left">celular</TableCell>
            <TableCell align="left">fechaNac</TableCell>
              ...
              <DialogContent dividers>
                  <GridContainer>
                    ...
                    <GridItem  xs={12} sm={12} md={12}>
                      Esta seguro que quiere eliminar el estudiante <strong>{consoleSeleccionada.nombre} {consoleSeleccionada.apPaterno} {consoleSeleccionada.apMaterno}</strong>?
                    </GridItem>
