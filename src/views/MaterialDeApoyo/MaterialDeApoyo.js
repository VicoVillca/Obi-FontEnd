...
const baseUrl = HOST.URL_BACK_END + "material";
...
export default function Olimpiada() {
  ...
  const handleChangeSearch = (event) => {
    ...
    if (1 > event.target.value.length) setResultado2(resultado);
      ...
      var val = resultado.filter((r) =>
        (r.titulo + r.subTitulo).includes(city)
      );
      setResultado2(val);
    }
  };
  ...
  const Insert = async (event) => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        enqueueSnackbar("Se agrego el material de apoyo!", {
          variant: "success",
        });
        getAll();
      })
  ...
  const Update = async (event) => {
    ...
    await axios
      .patch(
        baseUrl + "/" + consoleSeleccionada.idMaterial,
        JSON.stringify({
          titulo: consoleSeleccionada.titulo,
          subTitulo: consoleSeleccionada.subTitulo,
          tipo: consoleSeleccionada.tipo,
  ...
  const Delete = async (event) => {
    ...
    await axios
      .delete(baseUrl + "/" + consoleSeleccionada.idMaterial, header)
      .then((response) => {
        console.log(response);
        //showNotificationSuccess('success','Grupo guardado con exito');
            ...
            <GridContainer>
              <GridItem xs={12} sm={12} md={12}>
                <TextField
                  ...
                  type="text"
                  autoComplete="off"
                  value={consoleSeleccionada.titulo}
              ...
              <GridItem xs={12} sm={12} md={12}>
                <TextField
                  ...
                  type="text"
                  autoComplete="off"
                  value={consoleSeleccionada.subTitulo}
                  onChange={handleChangle}
                />
              </GridItem>
              ...
              <GridItem xs={12} sm={12} md={8}>
                <TextField
                  ...
                  type="url"
                  autoComplete="off"
                  value={consoleSeleccionada.archivo}
                  onChange={handleChangle}
                />
              </GridItem>
                  ...
                  <Box sx={{ minWidth: 20, marginTop: 2 }}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        value={consoleSeleccionada.tipo}
                        onChange={handleChangle}
                        defaultValue={0}
                      >
                        {tiposDeMaterial.map((row, index) => (
