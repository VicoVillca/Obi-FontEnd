...
export default function Olimpiada() {
  ...
  const handleChangeSearch = (event) => {
    ...
    if(event.target.value === 'Todos')
      ...
      var val = resultado.filter( 
        r => (r.departamento).includes(city)
      );
  ...
  const Insert=async(event)=>{
    ...
    const Update=async(event)=>{
      ...
      await axios.patch(baseUrl+"/"+consoleSeleccionada.idDistrito,
        JSON.stringify({
          "nombre":consoleSeleccionada.nombre,
          "departamento":consoleSeleccionada.departamento
        })
  ...
  const Delete=async(event)=>{
    ...
    await axios.delete(baseUrl+"/"+consoleSeleccionada.idDistrito
      ,header
    ).then(
      response => {
        console.log(response);
            ...
            >
              ...
              <DialogContent dividers>
                  <GridContainer>
                    ...
                    <GridItem  xs={12} sm={12} md={12}>
                      Esta seguro que quiere eliminar el distrito <strong>{consoleSeleccionada.nombre}</strong>?
                    </GridItem>
